...
automatically retried after the configured ``retry-unsuccessful-join-after`. The retries means that it
tries to contact all seed nodes and then join the node that answers first. The node that is first
in the list of seed nodes will join itself if it cannot contact any of the other seed nodes within the
